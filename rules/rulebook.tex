...
\documentclass[11pt,letterpaper,american,twocolumn]{article}
...
\fancyfoot[C]{Rulebook v0.2.6 — \DTMtoday}
\fancyfoot[R]{\thepage}
...
\item 5 per company
\end{itemize}
\item 120 wooden cubes
\begin{itemize}
\item 20 per company
\end{itemize}
\item Money
\begin{itemize}
\item Poker chips recommended
\end{itemize}
\item Hex value markers
\begin{itemize}
\item Rules describe stacked chips, but wooden cubes will do
\end{itemize}
\item Map
\begin{itemize}
\item Included file is scaled to mini-chips hex value markers. Adjust if
needed.
\end{itemize}
\end{itemize}
...
\begin{enumerate}
\item \label{enu:unissued-shares}One side has an empty circle on it, indicating
initial investor equity. These represent unissued shares that have
not been sold to raise company capital.
\begin{enumerate}
\item An issued share that has been rotated 90° indicates it has been leveraged
(\ref{subsec:Leverage-Share}).
\end{enumerate}
\item \label{enu:issued-shares}The opposite side has a red circle with
a bar through it, indicating an issued share. Once a share has been
issued, it may not be issued again.
\end{enumerate}
\item \label{enu:Share-control-own}A player \noun{controls} all shares
in their player area, but only issued shares that aren't leveraged
count towards final scoring.
\end{enumerate}
...
\begin{enumerate}
\item Put the company's charter above the map.
\item Place \$5 in the company’s treasury on the charter.
\end{enumerate}
...
\begin{enumerate}
\item Each player takes all shares of one company in the game and places
them in their player area: 4 on their unissued side, and 1 on its
issued side (\ref{enu:unissued-shares}).
...
\item Leverage Share\ref{subsec:Leverage-Share}
...
\item Pay \$4 x that value from the company treasury to the bank.
...
\begin{enumerate}
\item For each unissued or leveraged share the turn player controls, they
must pay \$1 to the share company’s treasury. This represents the
cost of retaining ownership that has not yet been sold to raise capital.
\begin{enumerate}
...
through any combination of networks to the target hex. Therefore,
if a company already has access to a given hex, it may not lease network
to said hex.
\item The cost of leasing network is the network value of the hexes used.
The acting company must pay this value to the owner(s) of the leased
network, equal to the value of their leased network.
\begin{enumerate}
...
\subsection{Leverage Share}\label{subsec:Leverage-Share}
\begin{enumerate}
\item When leveraging a share, the turn player selects one issued share
they control and rotates it 90° to indicate it has been leveraged
and immediately takes from the bank cash equal to the leveraged shares
current values (\ref{enu:Share-value}).
\end{enumerate}
...
\begin{enumerate}
\item Their cash
\item The Share Value of all issued, non-leveraged shares they control (\ref{enu:Share-control-own}).
\begin{enumerate}
